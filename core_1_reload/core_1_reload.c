...
volatile unsigned char setbackup[22] = {0x3C,0x16 ,0xE9 ,0x05 ,0xFA ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x00 ,0x3C ,0x00 ,0x00 ,0x05 ,0x00 ,0x00 ,0x03 ,0x3A ,0xC5 ,0x3E};
volatile unsigned char command[40];
volatile unsigned char commandPC[40];
//volatile unsigned char reply[2];
volatile unsigned char C2_reply[4] = {0};
...
//				puts_0((unsigned char *)C2_reply,4);
				...
				if(C2_reply[1]==tch)
				{
					switch(C2_reply[1])
					{
						case	1:	START_TIMER1;
						case	2:
						case	3:	if(C2_reply[1] == 3) STOP_TIMER1;
						case	4:
						case	5:
						case	6:
						case	7:
						case	8:	command[0] = head;
									command[11] = tail;
									command[1] = 4;
									command[2] = ~command[1];
